...
begin
  ...
  l := -10;
  p := 10;
  Print('Введите масштаб (рекомендуется от 20 до 40): ');
  ...
  while x <= p do
  begin
    y := (2 * (x * x * x) + (-1) * (x * x) + (1) * x + (17));
    x1 := round(x0 + x * mx);
    y1 := round(y0 + y * my);
    setpixel(x1, y1, clBlue);
    x += 0.001
  end;
  Print('Откройте "Во весь экран", а также повторите a, b, n: ');
